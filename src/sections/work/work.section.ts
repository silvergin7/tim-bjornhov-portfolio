...
import { getProjects, getExperiments } from '@/data/projects.data';
import type { Project } from '@/models/project';
import type { Experiment } from '@/data/projects.data';
...
const chartIcon = `<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>`;
...
const createExperimentCard = (exp: Experiment): string => `
  <article class="experiment-card">
    <div class="experiment-card__body">
      <span class="experiment-card__badge">${languageService.getText('experimentBadge')}</span>
      <div class="experiment-card__header">
        ${exp.imageSrc ? `
          <img class="experiment-card__logo experiment-card__logo--light" src="${exp.imageSrc.replace('.png', '-light.png')}" alt="${exp.imageAlt ?? ''}" />
          <img class="experiment-card__logo experiment-card__logo--dark" src="${exp.imageSrc}" alt="${exp.imageAlt ?? ''}" />
        ` : ''}
        <h2 class="experiment-card__title">${exp.title}</h2>
      </div>
      <p class="experiment-card__description">${exp.description}</p>
      <p class="experiment-card__disclaimer">${languageService.getText('experimentDisclaimer')}</p>
      <div class="experiment-card__footer">
        <ul class="project-card__tags">
          ${exp.tags.map((tag) => `<li class="project-card__tag">${tag}</li>`).join('')}
        </ul>
        <a class="experiment-card__chart" href="${exp.chartUrl}" target="_blank" rel="noopener noreferrer" aria-label="View chart on Dextools">${chartIcon}</a>
      </div>
    </div>
  </article>
`;
    ...
    <section id="work" class="page-section work-section">
      <div class="page-section__inner work-section__inner">
        ...
        <div class="work-section__projects">
          ...
          ${experiments.map(createExperimentCard).join('')}
        </div>
      </div>
    </section>
  `;
